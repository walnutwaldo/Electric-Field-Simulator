...
public class Line3D {
    ...
    public static final float LINE_THICKNESS = 0.0015f;

    private static final int[] DX = new int[]{-1, 0, 1, 0};
    private static final int[] DY = new int[]{0, 1, 0, -1};

    public static LineSeg getVisibleSeg(Matrix point1, Matrix point2) {
        if (Camera.visible(point1) && Camera.visible(point2)) return new LineSeg(point1, point2, null);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            Matrix inters = intersection(new LineSeg(point1, point2, null), new Plane(Camera.getTransformedPos(), normal));
            if (Camera.visible(inters)) ints.add(inters);
        }
        if (Camera.visible(point1)) return new LineSeg(point1, ints.get(0), null);
        if (Camera.visible(point2)) return new LineSeg(point2, ints.get(0), null);
        else if (!ints.isEmpty()) {
            for (int i = ints.size() - 1; i > 0; i--) if (approx(ints.get(i), ints.get(i - 1))) ints.remove(i);
            return new LineSeg(ints.get(0), ints.get(1), null);
        }
        return new LineSeg(null, null, null);
    }
